...
export function isWebSocketStale({
  ...
  return now - lastPongAt > staleMs && now - lastActivityAt > staleMs;
}
...
export function getReconnectDelayMs(attempt: number): number {
  ...
  return RECONNECT_DELAYS_MS[index] ?? 60_000;
}
...
export function classifyWebSocketPayload(
  payload: unknown,
): ControlResult | DispatchResult | InvalidResult {
  ...
  if (cmd === "directpush" || type === "msgchg") {
    const ack =
      record.needAck === true && typeof record.seq === "number"
...
export function deriveYZJWebSocketUrl(sendMsgUrl: string): string {
  ...
  return `${protocol}//${parsed.host}/xuntong/websocket?yzjtoken=${encodeURIComponent(token)}`;
}
